...
server <- function(input, output, session) {
  ...
  output$serie <- renderPlot({
    ...
    generar.serie(datosVec,
                  inicio = fecha.inicio(as.character(input$start)),
                  frecuencia = as.double(input$frequency))
    serie.tiempo(datosSerie)
  })

  fecha.inicio <- function(fecha = as.character(Sys.Date())) {
    startChr <- as.character(input$start)
    ifelse(is.null(startChr) || startChr == "" || is.na(startChr), as.character(Sys.Date()), startChr)
  }
  ...
  calcular.modelo <- reactive({
    generar.serie(datosVec, fecha.inicio(as.character(input$start)), as.double(input$frequency));
    switch(input$model,
